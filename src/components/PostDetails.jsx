...
//               </div>
//               <div
//                 className={`text-[18px] mb-6 leading-relaxed post-description ${
//                   isDarkMode ? "text-gray-200" : "text-black"
//                 }`}
//                 role="region"
//                 aria-label="Post description"
...
//         const postsArray = Array.isArray(allPostsRes) ? allPostsRes : [];
//         setAllPosts(postsArray);
//         const uniqueCategories = [
//           "All",
//           ...new Set(postsArray.map((post) => post.category || "General")),
//         ];
//         setCategories(uniqueCategories);
//         if (foundPostRes && isAuthenticated) {
//           const userId = localStorage.getItem("userId");
...
//       const updatedPost = postsArray.find((p) => p._id === postId);
//       setPost(updatedPost);
//       setComment("");
//       setCommentsPage(1);
//       toast.success("Comment added");
...
//   // Ensure latestStories and mostViewedPosts are only computed if allPosts is an array
//   const latestStories = Array.isArray(allPosts)
//     ? allPosts
//         .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
//         .slice(0, 5)
//     : [];
//   const mostViewedPosts = Array.isArray(allPosts)
//     ? allPosts
//         .sort((a, b) => (b.likes?.length || 0) - (a.likes?.length || 0))
//         .slice(0, 5)
//     : [];
...
//         .slice(0, showMoreRelated ? relatedPostsCount : 5)
//     : [];
//   const currentIndex = Array.isArray(allPosts)
//     ? allPosts.findIndex((p) => p._id === postId)
//     : -1;
//   const prevPost = currentIndex > 0 ? allPosts[currentIndex - 1] : null;
//   const nextPost =
//     currentIndex < allPosts.length - 1 && currentIndex !== -1
...
//                 onChange={(e) => setComment(e.target.value)}
//                 placeholder={placeholderText || fullPlaceholder}
//                 className={`w-full p-4 rounded-lg border focus:ring-2 focus:ring-blue-500 focus:outline-none resize-none h-24 shadow-sm transition-all duration-300 ${
//                   isDarkMode
//                     ? "bg-gray-800 text-gray-100 border-gray-700 placeholder-gray-400"
//                     : "bg-gray-100 text-gray-900 border-gray-200 placeholder-gray-500"
//                 }`}
...
//         <motion.button
//           onClick={scrollToCommentInput}
//           className="fixed lg:bottom-20 bottom-4 right-6 z-50 p-2 bg-blue-6
// 00 text-white rounded-full shadow-lg"
//           whileHover={{ scale: 1.1 }}
//           whileTap={{ scale: 0.9 }}
//           aria-label="Add a comment"
//         >
...
//         // Fetch Most Viewed Posts (assuming API supports sorting by reactions)
...
//         // Fetch Related Posts (filtered by hashtag or category)
//         const hashtag = foundPostRes.hashtags?.[0] || "";
//         const relatedPostsRes = await getPosts({ page: 1, limit: 10, hashtag });
//         const relatedPostsArray = Array.isArray(
//           relatedPostsRes.posts || relatedPostsRes
//         )
//           ? relatedPostsRes.posts || relatedPostsRes
//           : [];
//         setRelatedPosts(
...
//           "All",
//           ...new Set(
//             [...latestPostsArray, ...mostViewedArray, ...relatedPostsArray].map(
...
//     };
//     fetchData();
//   }, [postId, isAuthenticated, showMoreRelated, relatedPostsCount]);
...
//         const hashtags = foundPostRes.hashtags || [];
//         let relatedPostsArray = [];
//         if (hashtags.length > 0) {
//           // Fetch posts for each hashtag and combine results
...
//           "@type": "ListItem",
//           position: 2,
//           name: post?.hashtags?.[0] || "General",
//           item: `https://gossiphub.in/posts/hashtag/${
//             post?.hashtags?.[0]
...
//                 </div>
//               )}
//               {/* Optional: Keep category display for main post if needed */}
//               {post.category && (
//                 <div className="flex flex-wrap gap-2 mb-6">
//                   <span
//                     className={`px-3 py-1 text-xs font-medium ${
...
//     fetchMainPost();
//   }, [postId, isAuthenticated]);
//   // Fetch secondary data (latest stories, most viewed, related posts)
//   useEffect(() => {
