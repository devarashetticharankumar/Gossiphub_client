...
//         const fetchedPosts = await getPostsByCategory(decodedCategory, {
//           page: 1,
//           limit: 10,
//         });
//         const postsArray = Array.isArray(fetchedPosts.posts || fetchedPosts)
//           ? fetchedPosts.posts || fetchedPosts
//           : [];
//         setPosts(postsArray);
//         setHasMore(postsArray.length === 10);
//       } catch (err) {
//         toast.error("Failed to fetch posts for category");
//         console.error(err);
//       } finally {
//         setLoading(false);
//       }
...
//         (p) => !posts.some((existingPost) => existingPost._id === p._id)
//       );
...
//     }
//   }, [category, page, hasMore, postsLoading, posts]);
...
//   // SEO metadata
//   const pageTitle = `Posts in ${decodeURIComponent(category)} | GossipHub`;
//   const pageDescription = `Explore posts in the ${decodeURIComponent(
//     category
//   )} category on GossipHub. Discover the latest gossip and stories!`;
//   const pageUrl = `${window.location.origin}/posts/category/${category}`;
...
//       </Helmet>
//       {/* Header */}
//       <header className="fixed top-0 left-0 right-0 z-50 bg-red-600/90 backdrop-blur-md text-white shadow-md">
    ...
    >
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-red-600/90 backdrop-blur-md text-white shadow-lg">
        <div className="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
          ...
          <button
            onClick={toggleDarkMode}
            className="p-2 bg-red-700 rounded-full hover:bg-red-600 transition-colors"
            aria-label={
              isDarkMode ? "Switch to light mode" : "Switch to dark mode"
            }
          >
...
// // List of categories from the provided <option> elements
// const categories = [
//   "General",
//   "Tech News",
...
//     decodeURIComponent(category || "General")
//   );
//   const observerRef = useRef(null);
//   const sentinelRef = useRef(null);
...
//     navigate(`/posts/category/${encodeURIComponent(newCategory)}`);
//   };
...
//   // SEO metadata
//   const pageTitle = `Posts in ${decodeURIComponent(
//     category || "General"
//   )} | GossipHub`;
//   const pageDescription = `Explore posts in the ${decodeURIComponent(
//     category || "General"
//   )} category on GossipHub. Discover the latest gossip and stories!`;
//   const pageUrl = `${window.location.origin}/posts/category/${
...
//               }`}
//             >
//               No posts found in {decodeURIComponent(category || "General")}{" "}
//               category.
//             </motion.p>
//           ) : (
//             <>
...
//                           <div className="flex flex-wrap gap-2 mb-4">
//                             <Link
//                               to={`/posts/category/${encodeURIComponent(
//                                 post.category
//                               )}`}
//                               className={`px-3 py-1 text-xs font-medium ${
//                                 isDarkMode
