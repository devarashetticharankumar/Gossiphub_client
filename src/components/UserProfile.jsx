...
//           ? shortsRes.shorts
//           : [];
//         console.log("Raw shorts array:", shortsArray);
//         console.log("First short details:", shortsArray[0]);
//         console.log(
//           "Short author IDs:",
//           shortsArray.map((s) => s.user?._id)
//         );
//         const userShorts = shortsArray.filter((s) => s.user?._id === userId);
//         console.log("Filtered user shorts:", userShorts);
//         setShorts(shuffleArray(userShorts));
//         dataCache.current.shorts = userShorts;
//         setHasMoreShorts(shortsArray.length === 10);
//       } else {
//         setShorts(shuffleArray(dataCache.current.shorts));
...
//       fetchFollowing();
//     }
//   }, [user, fetchPosts, fetchShorts, fetchFollowers, fetchFollowing]);
...
//       }
//     }, 1000),
//     [pagePosts, hasMorePosts, postsLoading, user, posts]
//   );
...
//       }
//     }, 1000),
//     [pageShorts, hasMoreShorts, shortsLoading, user, shorts]
//   );

//   useEffect(() => {
//     if (
//       !sentinelRef.current ||
//       (!hasMorePosts && !hasMoreShorts) ||
//       (postsLoading && shortsLoading)
//     )
//       return;
...
//       (entries) => {
//         if (entries[0].isIntersecting) {
//           if (activeTab === "posts" && hasMorePosts) fetchMorePosts();
//           if (activeTab === "shorts" && hasMoreShorts) fetchMoreShorts();
//         }
//       },
//       { threshold: 0.1 }
//     );
...
//         );
//       } else if (type === "short") {
//         await deleteShort(itemId);
//         setShorts((prev) => prev.filter((short) => short._id !== itemId));
//         dataCache.current.shorts = dataCache.current.shorts.filter(
//           (short) => short._id !== itemId
//         );
//       }
//       toast.success(
...
//     } catch (err) {
//       const message = err.message || `Failed to delete ${type}`;
//       toast.error(message);
//     }
//   };
...
//   const openEditShort = (shortId) => {
//     navigate(`/shorts/${shortId}/edit`);
//     setMenuOpen(null);
//   };
...
//                                   e.preventDefault();
//                                   setMenuOpen(
//                                     menuOpen === short._id ? null : short._id
//                                   );
//                                 }}
//                                 className="p-1 rounded-full bg-gray-800/60 text-white hover:bg-gray-700/60 transition-colors duration-200"
...
//                     whileTap={{ scale: 0.95 }}
//                     onClick={() =>
//                       handleDeleteItem(itemToDelete._id, itemToDelete.type)
//                     }
//                     className="bg-red-600 text-white px-4 py-1.5 rounded-full hover:bg-red-700 transition-colors duration-200 text-sm font-medium"
//                     aria-label="Confirm deletion"
//                   >
  ...
  const fetchShorts = useCallback(async () => {
    try {
      ...
      const shortsArray = Array.isArray(shortsRes.shorts)
        ...
        : [];
      const userShorts = shortsArray.filter((s) => s.user?._id === userId);
      // Set new data only if different
      if (
        JSON.stringify(userShorts) !== JSON.stringify(dataCache.current.shorts)
      ) {
  ...
  const fetchMoreShorts = useCallback(
    throttle(async () => {
      ...
      try {
        ...
        setHasMoreShorts(shortsArray.length === 10); // Only true if full page is returned
      } catch (err) {
        console.error("More shorts fetch error:", err);
        toast.error("Failed to load more shorts");
      } finally {
            ...
            <AnimatePresence>
              {shortsLoading && shorts.length === 0 ? (
                ...
                <>
                  ...
                  >
                    {shorts.map((short) => {
                      console.log("Navigating to short:", short._id); // Debug log
                      return (
                        <motion.div
                          key={short._id}
                          initial={{ opacity: 0, scale: 0.98 }}
